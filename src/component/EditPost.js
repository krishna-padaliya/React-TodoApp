...
function EditPost({editPost , post}){
     const{id} = useParams()
     let po = post.find((po)=>po.id === parseInt(id))
     const [title , setTitle] = useState(po.title)
     const [content , setContent] = useState(po.content)
     const navigate = useNavigate()

     function handleSubmit(e){
        e.preventDefault()
        editPost(po.id ,title ,content)
        navigate("/")
     }
...
<div class="card shadow p-3 mb-5 rounded">
  <div class="card-header fs-3">Create New Post </div>
  <div class="card-body">
